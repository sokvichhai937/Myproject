...
function displayConversations() {
  ...
  if (conversations.length === 0) {
    conversationsList.innerHTML =
      '<div class="alert alert-info">មិនមានសារនៅឡើយទេ</div>';
    return;
  }
  ...
  conversationsList.innerHTML = conversations
    .map((conv) => {
      ...
      const preview =
        lastMsg.from === currentUser.username
          ? `អ្នក: ${lastMsg.content.substring(0, 30)}...`
          : lastMsg.content.substring(0, 30) + "...";

      return `
            <div class="list-group-item conversation-item" data-username="${conv.username}">
                <div class="d-flex align-items-center">
                    <a href="profile.html?user=${conv.username}" class="profile-image-small me-3 text-decoration-none" onclick="event.stopPropagation()">
                        ${
                          user.profileImage
  ...
  document.querySelectorAll(".conversation-item").forEach((item) => {
    item.addEventListener("click", function (e) {
      // Don't open chat if clicking on profile link
      if (e.target.closest('a[href^="profile.html"]')) {
        return;
      }
...
function openChat(otherUsername) {
  ...
  if (chatHeader) {
    chatHeader.innerHTML = `
            <div class="d-flex align-items-center">
                <a href="profile.html?user=${otherUsername}" class="profile-image-small me-2 text-decoration-none">
                    ${
                      otherUser.profileImage
                        ? `<img src="${otherUser.profileImage}" alt="${otherUser.fullName}">`
                        : `<div class="profile-placeholder">${otherUser.fullName.charAt(0)}</div>`
                    }
                </a>
                <div>
                    <h6 class="mb-0">
                        <a href="profile.html?user=${otherUsername}" class="text-decoration-none text-dark">
