...
function displayPosts(containerId, filterUsername = null) {
  ...
  if (posts.length === 0) {
    container.innerHTML =
      '<div class="alert alert-info">មិនមានប្រកាសនៅឡើយទេ</div>';
    return;
  }
                ...
                <div class="card-body">
                    ...
                    <div class="comments-section" id="comments-${post.id}" style="display: none;">
                        <div class="comments-list mb-3" id="comments-list-${post.id}">
                            ${comments
                              .map((comment) => {
                                const commentUser = findUserByUsername(
                                  comment.username,
                                );
                                        ...
                                        <div class="d-flex align-items-start">
                                            <div class="profile-image-tiny me-2">
                                                ${
                                                  commentUser.profileImage
                                                    ? `<img src="${commentUser.profileImage}" alt="${commentUser.fullName}">`
                                            ...
                                            <div class="flex-grow-1">
                                                <div class="d-flex justify-content-between align-items-start">
                                                    <small>
                                                        <strong>
                                                            <a href="profile.html?user=${comment.username}" class="text-decoration-none text-dark">
                                                                ${escapeHtml(commentUser.fullName)}
                                                            </a>
                                                        </strong>
                                                    </small>
                                                    <div>
  ...
  document.querySelectorAll(".comment-input").forEach((input) => {
    input.addEventListener("keypress", function (e) {
      if (e.key === "Enter") {
        const postId = this.dataset.postId;
        document
          .querySelector(`.add-comment-btn[data-post-id="${postId}"]`)
          .click();
      }
    });
  });
