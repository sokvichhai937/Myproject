...
function getNotificationText(notification) {
    ...
    switch (notification.type) {
        case 'like':
            return `${fromUserName} បានចូលចិត្តប្រកាសរបស់អ្នក`;
        case 'comment':
            return `${fromUserName} បានបញ្ចេញមតិលើប្រកាសរបស់អ្នក`;
        case 'follow':
            return `${fromUserName} បានចាប់ផ្តើមតាមដានអ្នក`;
        default:
            return 'ជូនដំណឹងថ្មី';
    }
}
...
function displayNotifications() {
    ...
    if (notifications.length === 0) {
        container.innerHTML = '<div class="alert alert-info">មិនមានជូនដំណឹងនៅឡើយទេ</div>';
        return;
    }
            ...
            <div class="list-group-item ${notification.read ? '' : 'bg-light'} notification-item" data-notification-id="${notification.id}">
                <div class="d-flex align-items-start">
                    <div class="profile-image-small me-3">
                        ${fromUser.profileImage ? `<img src="${fromUser.profileImage}" alt="${fromUser.fullName}">` : 
